...
The correct intercept is `r gamma`, the linear component is `r beta`, and the squared component is `r alpha`. Our estimated parameters are not far off.  When we fitted this model, it doesn't matter if we used the closure or the global formula. The `data` parameter overscopes them anyway.
